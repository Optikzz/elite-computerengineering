...
local function SetJobBlip(title)
    ...
    SetBlipSprite(JobBlip, 181)
    SetBlipDisplay(JobBlip, 4)
    SetBlipScale(JobBlip, 0.7)
    SetBlipAsShortRange(JobBlip, true)
    SetBlipColour(JobBlip, 47)
...
local function SetWorkBlip(d)
    ...
    TriggerEvent('elite-computerjob:client:Job')
end
...
RegisterNetEvent('elite-computerjob:client:VehPick', function()
    local choice = math.random(1, #Config.JobVehicles)
    ElecVeh = Config.JobVehicles[choice]
    TriggerEvent('elite-computerjob:client:SpawnVehicle', ElecVeh)
end)
                        ...
                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                            if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                if VehicleCheck(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                    ...
                                    exports['mythic_notify']:DoHudText('error', 'Това не е автомобил от LifeInvader!')
                                end
                            else
                                exports['mythic_notify']:DoHudText('error', 'Трябва да си шофьор на това МПС!')
                            end
                        end
                    end  
                end
...
RegisterNetEvent('elite-computerjob:client:SpawnVehicle', function(vehicleInfo)
...
RegisterNetEvent('elite-computerjob:client:Job', function(k, v)
    local inRange = false
    CompleteRepairs = 0
    while true do
        Wait(0)
                ...
                -- if PlayerJob.name == "computer" then
                    if #(pos - vector3(v.x, v.y, v.z)) < 10 then
                        ...
                        if #(pos - vector3(v.x, v.y, v.z)) < 1.5 then
                            DrawText3D(v.x, v.y, v.z, "~g~E~w~ - Поправи компютъра")
                            if IsControlJustReleased(0, 38) then
                                QBCore.Functions.Progressbar("repair_work", "Поправяне на компютъра...", 15000, false, true, {
                                    ...
                                    if CompleteRepairs <= 4 then
                                        exports['mythic_notify']:DoHudText('inform', 'Компютърът е поправен, продължавай към следващата локация!')
                                        TriggerServerEvent("elite-computerjob:server:Reward"); 
                                        TriggerServerEvent("elite-computerjob:server:Payslip");
                                    else 
                                        exports['mythic_notify']:DoHudText('inform', 'Работата е приключена! Върни се в LifeInvader ,за вземеш нови локации!')
                                        TriggerServerEvent("elite-computerjob:server:Payslip");
                                    end
                                    RemoveBlip(v.BlipId)
                                    ...
                                    exports['mythic_notify']:DoHudText('error', 'Отказано')
                                end)
                            end  
                        end
